...
export async function getStaticProps() {
	const { frontmatter, source } = await getFileContent('publications.mdx')
...
export default function Publications({ meta, source }: Props) {
	return (
		<article className="prose dark:prose-dark max-w-full">
			...
			<h1>{meta.title}</h1>
			<MDXRemote {...source} components={{ Link, Talk, Project, Publication, Presentation }} />
		</article>
	)
}
